...
function JumpPoint(matrix) {
  ...
  this.identifySuccessors = function(cube) {
    var neighbours = null;
    if (this.matrix.cornerWalk == CornerWalk.Diagonal) {
      neighbours = this.findNeighboursDiagonal(cube);
    } else if (this.matrix.cornerWalk == CornerWalk.Orthogonal) {
      neighbours = this.findNeighboursOrthogonal(cube);
    }
    ...
    for (var i = 0, l = neighbours.length; i < l; i++) {
      ...
      var jumpPoint = null;
      if (this.matrix.cornerWalk == CornerWalk.Diagonal) {
        jumpPoint = this.jumpDiagonal(neighbour[0], neighbour[1], cube.x, cube.y);
      } else if (this.matrix.cornerWalk == CornerWalk.Orthogonal) {
        jumpPoint = this.jumpOrthogonal(neighbour[0], neighbour[1], cube.x, cube.y);
      }
  ...
  this.findNeighboursDiagonal = function(cube) {
    var neighbours = [];
  ...
  this.jumpDiagonal = function(x, y, fx, fy) {
    var dx = x - fx;
    var dy = y - fy;
    ...
    if (dx !== 0 && dy !== 0) { // along the diagonal
      ...
      if (this.jumpDiagonal(x + dx, y, x, y) || this.jumpDiagonal(x, y + dy, x, y)) {
        return [x, y];
      }
    ...
    if (this.matrix.isCubeWalkable(x + dx, y) || this.matrix.isCubeWalkable(x, y + dy)) {
      return this.jumpDiagonal(x + dx, y + dy, x, y);
    } else {
      return null;
    }
  }

  this.findNeighboursOrthogonal = function(cube) {
    var neighbours = [];
  ...
  this.jumpOrthogonal = function(x, y, fx, fy) {
    var dx = x - fx;
    var dy = y - fy;
    var fixedIndex = fixDirection(dx, dy);
    ...
    if (dx !== 0) {
      if ((this.matrix.isCubeWalkable(x, y - 1) && !this.matrix.isCubeWalkable(x - dx, y - 1)) || (this.matrix.isCubeWalkable(x, y + 1) && !this.matrix.isCubeWalkable(x - dx, y + 1))) {
        return [x, y];
      }
    } else if (dy !== 0) {
      if ((this.matrix.isCubeWalkable(x - 1, y) && !this.matrix.isCubeWalkable(x - 1, y - dy)) || (this.matrix.isCubeWalkable(x + 1, y) && !this.matrix.isCubeWalkable(x + 1, y - dy))) {
        return [x, y];
      }
      //When moving vertically, must check for horizontal jump points
      if (this.jumpOrthogonal(x + 1, y, x, y) || this.jumpOrthogonal(x - 1, y, x, y)) {
        return [x, y];
      }
    } else {
    ...
    return this.jumpOrthogonal(x + dx, y + dy, x, y);
  }
}
